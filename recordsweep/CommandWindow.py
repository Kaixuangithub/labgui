...
class CommandWindow(QWidget):
    ...
    def __init__(self,availiable_ports=["GPIB0::" +str(i) for i in range(30)],parent=None):
        ...
        [self.INSTRUMENT_TYPES,self.AVAILABLE_PARAMS,self.UNITS]=readconfigfile.get_drivers(readconfigfile.get_drivers_path())
    ...
    def set_lists(self,num_channels):
        ...
        #TickBoxes
        self.color_btn_list=[]
        #Empty text boxes called Line Edit where the user can write which ever title for plotting purposes
        ...
        if self.num_channels>0:
            ...
            if first_init:
                #create the button 'Connect' and adds it at the end of the instrument list
                self.bt_connecthub=QtGui.QPushButton("Connect",self)
                self.bt_connecthub.setEnabled(False)
                self.connect(self.bt_connecthub,SIGNAL("clicked()"),self.bt_connecthub_clicked)
                ...
                spacer_item = QSpacerItem(20, 183, QSizePolicy.Minimum, QSizePolicy.Expanding)
                self.vertical_layout.addItem(spacer_item)
                self.setLayout(self.vertical_layout)
            else:
                #add the button 'Connect' at the end of the instrument list
                self.grid.addWidget(self.bt_connecthub, self.num_channels+1,3,1,1)
                #this is an inside class defined function that clear all the widgets on the windows as well as emptying the lists
        else:
            if first_init:
                pass
            else:
...
#        print "line created "+msg
        ...
        self.create_lineedit(self.param_name_le_list, 1)
        self.create_combobox("Instr name",self.instr_name_cbb_list, 2, label_text = "Type")
        self.create_combobox("Port",self.port_cbb_list, 3, label_text = "GPIB/COM Port")
        self.create_combobox("Param",self.param_cbb_list, 4, label_text = "Measurement")
    ...
    def remove_lastline(self):
        ...
        combo_array=[self.color_btn_list,self.param_name_le_list,self.instr_name_cbb_list,self.port_cbb_list,self.param_cbb_list]        
...
#            print item
    ...
    def create_lineedit(self,le_list, col, label_text = "Name"):
        index=len(le_list)        
        le=QLineEdit(self)
        le.setObjectName(_fromUtf8("param_name_le_list "+str(index)))
    ...
    def create_tickbox(self,cb_list, col, label_text = "Color"):
        ...
        index=len(cb_list)
        cb=QCheckBox("",self)
        cb.setObjectName(_fromUtf8("checkBox "+str(index)))
    ...
    def create_combobox(self,name,cbb_list, col, label_text = "label"):
        index=len(cbb_list)
        cbb=QComboBox(self)
        cbb.setObjectName(_fromUtf8("comboBox "+name+" "+str(index)))
        cbb.setStyleSheet ("QComboBox::drop-down {border-width: 0px;} QComboBox::down-arrow {image: url(noimg); border-width: 0px;}")
        if name=='Instr name':
            cbb.addItems(self.INSTRUMENT_TYPES)
            cbb.setCurrentIndex(cbb.findText("TIME"))
            self.connect(cbb, SIGNAL("currentIndexChanged(int)"), self.combobox_handler)
        if name =="Param":
            cbb.addItems(self.AVAILABLE_PARAMS['TIME'])
            cbb.setCurrentIndex(cbb.findText("dt"))
            self.connect(cbb, SIGNAL("currentIndexChanged(int)"), self.combobox_unit_handler)
        #add the newly created widget to the list and tothe layout
        cbb_list.append(cbb) 
    ...
    def combobox_handler(self):
        ...
        if instr_name == "TIME":
            ...
            if box_index==self.num_channels-2 and self.num_channels>2:
                #if this is the last one of the list then remove it, only if the one before is also a TIME
                if self.instr_name_cbb_list[box_index+1].currentText()=="TIME":
                    self.remove_lastline()
        elif instr_name == "FridgeClient":
            port_box.clear()                
        else:
            #One could think of a solution for writing only the port that are still availiable
            port_box.clear()
            port_box.addItems(self.AVAILABLE_PORTS)
            #if this is the last one of the list then add a combobox into the list
        ...
        #After changes the user should be able to update the instrumentHub by clicking on the button 'Connect'
...
#        pass
    ...
    def collect_device_info(self):
        ...
        combo_array=[self.instr_name_cbb_list,self.port_cbb_list,self.param_cbb_list]
        device_info=[[],[],[]]
...
#            print self.INSTRUMENT_TYPES
    ...
    #                print instr_type
                    ...
                    if instr_type in self.INSTRUMENT_TYPES:           
                        ...
                        self.instr_name_cbb_list[idx].setCurrentIndex(self.instr_name_cbb_list[idx].findText(instr_type))
    ...
    #                        print port in self.AVAILABLE_PORTS
                            ...
                            if port in self.AVAILABLE_PORTS:
                                self.port_cbb_list[idx].setCurrentIndex(self.port_cbb_list[idx].findText(port))
    #                            print port
    #                            print self.port_cbb_list[idx].findText(port)
                            else:
                                print instr_type
                                self.port_cbb_list[idx].addItem(QIcon("not_found.png"), port)
                                self.port_cbb_list[idx].setCurrentIndex(self.instr_name_cbb_list[idx].count()-1)
                                
                        self.param_cbb_list[idx].clear()
                        self.param_cbb_list[idx].addItems(self.AVAILABLE_PARAMS[instr_type])
    
                        param = settings[3].strip()
                        if param in self.AVAILABLE_PARAMS[instr_type]:                       
                            self.param_cbb_list[idx].setCurrentIndex(self.param_cbb_list[idx].findText(param))  
                    idx +=1
            settings_file.close()
            self.remove_lastline()
            self.grid.addWidget(self.bt_connecthub, self.num_channels+2,3,1,1)
    ...
    def save_settings(self, fname):
        ...
        for idx in range (self.num_channels):
            ...
            settings_file.write(self.instr_name_cbb_list[idx].currentText() +', ')
