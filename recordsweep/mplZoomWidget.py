...
#        self.figure.canvas.draw()     
        ...
        self.emit(QtCore.SIGNAL("limits_changed(int,PyQt_PyObject)"),3,np.array(selection_limits))
    ...
    def mousePressEvent(self, event):
        if event.button() == QtCore.Qt.LeftButton:
            if self.mouseMode == self.PAN_MODE:
                ...
                self.__yScale_L = calculate_scaling(self.__YLim_0_L, self.height())
                self.__yScale_R = calculate_scaling(self.__YLim_0_R, self.height())
                ...
                self.zoom_click_X,self.zoom_click_Y = inv.transform((event.x(), self.height() - event.y()))
                #self.grab_line_closest_to(event.x(), event.y())
                # draw a rectangle indicating the new zoom area                
                self.zoom_rectangle = self.axes.add_patch(mpl.patches.Rectangle((self.zoom_click_X,self.zoom_click_Y),0,0, transform = self.axes.transAxes))
...
#            self.emit(QtCore.SIGNAL("mousePressed(PyQt_PyObject)"), [self.__mousePressX, self.__mousePressY]) 
        ...
        if event.buttons() == QtCore.Qt.LeftButton:
            ...
            if self.mouseMode == self.PAN_MODE:
                if self.control_X_on:   
                    new_lim = convert_lim(self.axes.xaxis, self.__XLim_0 + self.__xScale * (event.x() - self.__mousePressX))
                    self.axes.set_xlim(new_lim)
                if self.control_Y_on:
                    new_lim = convert_lim(self.axes.yaxis, self.__YLim_0_L + self.__yScale_L * (event.y() - self.__mousePressY))             
                    self.axes.set_ylim(new_lim)
                if self.control_R_on  and self.usingR:
                    new_lim = convert_lim(self.axesR.yaxis, self.__YLim_0_R + self.__yScale_R * (event.y() - self.__mousePressY))             
                ...
                self.zoom_rectangle.set_bounds(X,Y, self.zoom_click_X-X, self.zoom_click_Y-Y)
                self.figure.canvas.draw()
    ...
    def mouseReleaseEvent(self, event):
        ...
        if True: #event.buttons() == QtCore.Qt.LeftButton:
            ...
            if self.mouseMode == self.ZOOM_MODE or self.mouseMode == self.SELECT_MODE:
                ...
                if self.mouseMode == self.ZOOM_MODE:               
                    ...
                    self.zoom_rectangle.remove()
                    self.select_rectangle.set_bounds(x_min, y_min,x_max-x_min,y_max-y_min) 
        ...
        self.emit(QtCore.SIGNAL("limits_changed(int,PyQt_PyObject)"),self.mouseMode,np.array(selection_limits))
    ...
    def get_selection_limits(self, x1, x2, y1, y2):
        ...
        #x_min etc. are in the figure coordinates (or the exponent coordinates for log scale)            
    ...
    def wheelEvent(self, event):
        ...
        if  do_scale_X:
            if ax.yaxis.get_scale() == 'log':
                ...
                ax.set_xlim(10**(X - zoom * (X - xlim[0])), 10**(X - zoom * (X - xlim[1]))) 
            else:                           
                xlim = ax.get_xlim()
                X = (xlim[0] + X_0 * (xlim[1]-xlim[0]))   
        ...
        for ax, on in zip ([self.axes, self.axesR], [do_scale_Y, do_scale_R]): 
            if on:
                if ax.yaxis.get_scale() == 'log':
                    #scale the exponents of the scale linearly instead when it's a log scale
                    ylim = np.log10(ax.get_ylim())
                    Y = (ylim[0] + Y_0 * (ylim[1]-ylim[0]))
                    ax.set_ylim(10**(Y - zoom * (Y - ylim[0])), 10**(Y - zoom * (Y - ylim[1]))) 
                else:    
                    ylim = ax.get_ylim()
                    Y = (ylim[0] + Y_0 * (ylim[1]-ylim[0]))
        ...
        self.rescale_and_draw()
        self.emit(QtCore.SIGNAL("limits_changed(int,PyQt_PyObject)"),-1,np.array([self.axes.get_xlim(),self.axes.get_ylim()]))
